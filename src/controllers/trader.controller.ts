...
export const getTraderLoanApplications = async (req: Request, res: Response) => {
  try {
    const traderId = (req as any).user.id;
...
export const traderApproveLoan = async (req: Request, res: Response) => {
  try {
    ...
    if (!approvedAmount || !repaymentDays || !voucherType) {
      return res.status(400).json({
        success: false,
        message: "Approved amount, repayment days, and voucher type are required"
      });
    }

    // Validate voucher type
    const validVoucherTypes = ["DISCOUNT_10", "DISCOUNT_20", "DISCOUNT_50", "DISCOUNT_80", "DISCOUNT_100"];
    ...
    const result = await traderApproveLoanService(traderId, loanId, {
      ...
      voucherType: voucherType as "DISCOUNT_10" | "DISCOUNT_20" | "DISCOUNT_50" | "DISCOUNT_80" | "DISCOUNT_100",
      notes
    });
...
export const getTraderTransactionHistory = async (req: Request, res: Response) => {
  try {
    const traderId = (req as any).user.id;
    ...
    const transactions = await getTraderTransactionHistoryService(traderId, filters);
