...
const activityMonitoring_service_1 = require("./activityMonitoring.service");
const analyticsEarnings_service_1 = require("./analyticsEarnings.service");
const farmingProfile_service_1 = require("./farmingProfile.service");
const location_service_1 = require("./location.service");
const pinManagement_service_1 = require("./pinManagement.service");
const profileManagement_service_1 = require("./profileManagement.service");
...
function paginateLocationList(items, page, limit = 6) {
...
async function handleLocationUpdate(parts, session, lang, phoneNumber) {
    ...
    if (session.locationStep === "province") {
        ...
        if (currentInput === "99") {
            ...
            return buildLocationMenu(provinces, session.locationPage, "selectProvince", lang, true);
        }
        if (currentInput === "98") {
        ...
        return buildLocationMenu(provinces, session.locationPage || 1, "selectProvince", lang, true);
    }
    ...
    if (session.locationStep === "district") {
        ...
        if (currentInput === "99") {
            session.locationPage = (session.locationPage || 1) + 1;
            return buildLocationMenu(districts, session.locationPage, "selectDistrict", lang, true);
        }
        if (currentInput === "98") {
            session.locationPage = Math.max((session.locationPage || 1) - 1, 1);
            return buildLocationMenu(districts, session.locationPage, "selectDistrict", lang, true);
        ...
        return buildLocationMenu(districts, session.locationPage || 1, "selectDistrict", lang, true);
    }
    ...
    if (session.locationStep === "sector") {
        ...
        if (currentInput === "99") {
            session.locationPage = (session.locationPage || 1) + 1;
            return buildLocationMenu(sectors, session.locationPage, "selectSector", lang, true);
        }
        if (currentInput === "98") {
            session.locationPage = Math.max((session.locationPage || 1) - 1, 1);
            return buildLocationMenu(sectors, session.locationPage, "selectSector", lang, true);
        ...
        return buildLocationMenu(sectors, session.locationPage || 1, "selectSector", lang, true);
    }
    ...
    if (session.locationStep === "cell") {
        ...
        if (currentInput === "99") {
            session.locationPage = (session.locationPage || 1) + 1;
            return buildLocationMenu(cells, session.locationPage, "selectCell", lang, true);
        }
        if (currentInput === "98") {
            session.locationPage = Math.max((session.locationPage || 1) - 1, 1);
            return buildLocationMenu(cells, session.locationPage, "selectCell", lang, true);
        ...
        return buildLocationMenu(cells, session.locationPage || 1, "selectCell", lang, true);
    }
    ...
    if (session.locationStep === "village") {
        ...
        if (currentInput === "99") {
            session.locationPage = (session.locationPage || 1) + 1;
            return buildLocationMenu(villages, session.locationPage, "selectVillage", lang, true);
        }
        if (currentInput === "98") {
            session.locationPage = Math.max((session.locationPage || 1) - 1, 1);
            return buildLocationMenu(villages, session.locationPage, "selectVillage", lang, true);
...
0. ${getTranslation(lang, "back")}`;
        ...
        return buildLocationMenu(villages, session.locationPage || 1, "selectVillage", lang, true);
    }
    ...
    if (session.locationStep === "confirm_location") {
        if (currentInput === "1") {
            ...
            try {
                const success = await profileManagement_service_1.ProfileManagementService.updateLocation(phoneNumber, {
...
function showLanguageSelection(lang = "KINY") {
...
async function handleBackNavigation(session, lang) {
...
function showMainMenu(lang = "KINY") {
...
function showAccountMenu(lang = "KINY") {
...
function buildProductMenu(products, page = 1, lang = "KINY") {
...
async function buildCategoryMenu(lang = "KINY", page = 1) {
...
async function verifyUserPin(phoneNumber, pin) {
    try {
        ...
        const isValid = await (0, password_1.comparePassword)(pin, farmer.password);
...
async function handleRegistration(parts, session, lang, phoneNumber, sessionId) {
    console.log(`Registration flow - Step: ${session.locationStep}, Parts: ${JSON.stringify(parts)}`);
    const currentInput = parts[parts.length - 1];
    // Handle province selection
    if (session.locationStep === "province") {
        const provinces = location_service_1.LocationValidationService.getAllProvinces();
        if (currentInput === "99") {
            session.locationPage = (session.locationPage || 1) + 1;
            return buildLocationMenu(provinces, session.locationPage, "selectProvince", lang, false);
        }
        if (currentInput === "98") {
            session.locationPage = Math.max((session.locationPage || 1) - 1, 1);
            return buildLocationMenu(provinces, session.locationPage, "selectProvince", lang, false);
        ...
        return buildLocationMenu(provinces, session.locationPage || 1, "selectProvince", lang, false);
    }
    // Handle district selection
    if (session.locationStep === "district") {
        const districts = location_service_1.LocationValidationService.getDistrictsByProvince(session.selectedProvince);
    ...
    if (session.locationStep === "sector") {
        ...
        if (currentInput === "99") {
            session.locationPage = (session.locationPage || 1) + 1;
            return buildLocationMenu(sectors, session.locationPage, "selectSector", lang, false);
        }
        if (currentInput === "98") {
            session.locationPage = Math.max((session.locationPage || 1) - 1, 1);
            return buildLocationMenu(sectors, session.locationPage, "selectSector", lang, false);
        ...
        return buildLocationMenu(sectors, session.locationPage || 1, "selectSector", lang, false);
    }
    // Handle cell selection
    if (session.locationStep === "cell") {
        const cells = location_service_1.LocationValidationService.getCellsBySector(session.selectedProvince, session.selectedDistrict, session.selectedSector);
        ...
        if (currentInput === "99") {
            session.locationPage = (session.locationPage || 1) + 1;
            return buildLocationMenu(cells, session.locationPage, "selectCell", lang, false);
        }
        if (currentInput === "98") {
            session.locationPage = Math.max((session.locationPage || 1) - 1, 1);
            return buildLocationMenu(cells, session.locationPage, "selectCell", lang, false);
        ...
        return buildLocationMenu(cells, session.locationPage || 1, "selectCell", lang, false);
    }
    // Handle village selection
    if (session.locationStep === "village") {
        const villages = location_service_1.LocationValidationService.getVillagesByCell(session.selectedProvince, session.selectedDistrict, session.selectedSector, session.selectedCell);
        ...
        if (currentInput === "99") {
            session.locationPage = (session.locationPage || 1) + 1;
            return buildLocationMenu(villages, session.locationPage, "selectVillage", lang, false);
        }
        if (currentInput === "98") {
            session.locationPage = Math.max((session.locationPage || 1) - 1, 1);
            return buildLocationMenu(villages, session.locationPage, "selectVillage", lang, false);
        ...
        return buildLocationMenu(villages, session.locationPage || 1, "selectVillage", lang, false);
    }
    // Handle PIN creation
...
${getTranslation(lang, "createPin")}`;
            ...
            // Register the user
            try {
                const hashedPin = await (0, password_1.hashPassword)(session.password);
...
async function getSystemStatus(lang) {
    try {
        const status = await support_service_1.SupportService.getSystemStatus();
...
async function getFAQContent(lang, faqIndex) {
    try {
        const faqs = support_service_1.SupportService.getFAQs(lang);
...
async function getIncomeSummaryDisplay(phoneNumber, lang) {
    try {
        const summary = await analyticsEarnings_service_1.AnalyticsEarningsService.getIncomesSummary(phoneNumber);
        if (!summary) {
...
${getTranslation(lang, "submitProductsFirst")}`;
        ...
        response += `${getTranslation(lang, "thisMonth")}: ${summary.thisMonth || 0} RWF\n`;
        response += `${getTranslation(lang, "lastMonth")}: ${summary.lastMonth || 0} RWF\n`;
        response += `${getTranslation(lang, "yearToDate")}: ${summary.yearToDate || 0} RWF\n`;
        response += `${getTranslation(lang, "avgPerSubmission")}: ${summary.avgPerSubmission || 0} RWF`;
        return response;
    }
    catch (error) {
...
async function showSecurityMenu(lang) {
...
async function showUpdateProfileMenu(lang) {
...
async function showFarmingProfileMenu(lang) {
...
async function showEarningsDashboardMenu(lang) {
...
async function getProductPurchasePrice(productName) {
    try {
...
4. ${getTranslation(lang, "exit")}`;
        ...
        if (!session.languageSelected && !userExists) {
            ...
            if (selectedLanguage) {
                ...
                session.locationPage = 1;
                const provinces = location_service_1.LocationValidationService.getAllProvinces();
                return buildLocationMenu(provinces, 1, "selectProvince", selectedLanguage, false);
            }
            else {
                return showLanguageSelection(lang);
            }
        ...
        if (!userExists && session.mode === "register") {
            return await handleRegistration(parts, session, lang, phoneNumber, sessionId);
        }
...
${await buildCategoryMenu(lang, session.categoryPage || 1)}`.replace("CON ", "");
                        }
                    }
                    ...
                    if (parts.length === 3) {
                        ...
                        if (productChoice === "99") {
                            ...
                            const productNames = products.map((product) => product.productName);
                            const paginated = paginateLocationList(productNames, session.productPage);
                            let productMenu = `CON ${getTranslation(lang, "selectProduct")}\n`;
                            paginated.items.forEach((product, index) => {
                                const itemNumber = (paginated.currentPage - 1) * 6 + index + 1;
                                productMenu += `${itemNumber}. ${product}\n`;
                        ...
                        if (productIndex >= 0 && productIndex < products.length) {
                            ...
                            return `CON ${getTranslation(lang, "enterQuantity")} (${session.selectedProductUnit})
0. ${getTranslation(lang, "back")}`;
                        }
                        else {
                            const paginated = paginateLocationList(products.map((product) => product.productName), session.productPage || 1);
...
0. ${getTranslation(lang, "back")}`;
                        ...
                        const purchasePrice = await getProductPurchasePrice(session.selectedProduct);
...
0. ${getTranslation(lang, "back")}`;
                            ...
                            try {
                                await support_service_1.SupportService.submitSupportTicket(phoneNumber, {
                                    category: "TECHNICAL",
...
0. ${getTranslation(lang, "back")}`;
                            ...
                            const pinResult = await verifyUserPin(phoneNumber, pinInput);
                            if (!pinResult.isValid) {
                                return `END ${getTranslation(lang, pinResult.message || "incorrectPasswordSubmissions")}`;
                            }
                            try {
                                ...
                                let response = `END ${getTranslation(lang, "recentSubmissions")}\n\n`;
                                submissions.forEach((submission, index) => {
                                    response += `${index + 1}. ${submission.productName}\n`;
                                    response += `   ${submission.submittedQty}kg @ ${submission.wishedPrice} RWF/kg\n`;
                                    response += `   ${getTranslation(lang, "status")}: ${getTranslation(lang, submission.status)}\n\n`;
                                });
...
0. ${getTranslation(lang, "back")}`;
                            ...
                            if (parts.length === 5 &&
                                ...
                                try {
                                    const success = await profileManagement_service_1.ProfileManagementService.updatePhoneNumber(phoneNumber, session.newPhoneNumber, verificationCode);
                                    if (success) {
                        ...
                        if (parts[2] === "2") {
                            ...
                            return await handleLocationUpdate(parts, session, lang, phoneNumber);
                        }
...
0. ${getTranslation(lang, "back")}`;
                            ...
                            if (parts[3] === "1") {
                                ...
                                try {
                                    await profileManagement_service_1.ProfileManagementService.updateCommunicationPreferences(phoneNumber, {
                                    ...
                                    return `END ${getTranslation(lang, "communicationUpdateSuccess")}`;
                                }
                                catch (error) {
                                ...
                                if (freqChoice in frequencyMap) {
                                    try {
                                        await profileManagement_service_1.ProfileManagementService.updateCommunicationPreferences(phoneNumber, {
                                            smsNotifications: farmer.smsNotifications,
                                            language: farmer.preferredLanguage,
                                            notificationFrequency: frequencyMap[freqChoice],
                                        });
                                        return `END ${getTranslation(lang, "communicationUpdateSuccess")}`;
                                    }
                                    catch (error) {
                    ...
                    if (parts[1] === "3") {
                        ...
                        if (parts[2] === "1") {
                            ...
                            if (parts.length === 4 &&
                                ...
                                if (categoryIndex >= 0 && categoryIndex < categories.length) {
                                    ...
                                    const products = await getProductsByCategory(session.selectedCategoryId);
                                    let productMenu = `CON ${getTranslation(lang, "selectCrops")}\n`;
                        ...
                        if (parts[2] === "2") {
                            if (parts.length === 3) {
                                ...
                                return `CON ${getTranslation(lang, "enterFarmSize")} (${getTranslation(lang, "hectares")})
0. ${getTranslation(lang, "back")}`;
                            }
                            if (parts.length === 4 &&
                                session.farmingProfileStep === "farm_info") {
...
0. ${getTranslation(lang, "back")}`;
                                ...
                                return `CON ${getTranslation(lang, "enterExperience")} (${getTranslation(lang, "years")})
0. ${getTranslation(lang, "back")}`;
                            }
                            if (parts.length === 5 &&
                                session.farmingProfileStep === "farm_experience") {
...
0. ${getTranslation(lang, "back")}`;
                                ...
                                try {
                                    await farmingProfile_service_1.FarmingProfileService.updateFarmInformation(phoneNumber, {
                            ...
                            if (parts.length === 4 &&
                                ...
                                if (paymentMethodChoice in paymentMethodMap) {
                                    session.paymentMethod =
                                        paymentMethodMap[paymentMethodChoice];
...
0. ${getTranslation(lang, "back")}`;
                                ...
                                try {
                                    await farmingProfile_service_1.FarmingProfileService.updateBusinessPreferences(phoneNumber, {
                        ...
                        if (parts[2] === "4") {
                            try {
                                const profile = await farmingProfile_service_1.FarmingProfileService.getFarmingProfile(farmer.id);
...
${getTranslation(lang, "pleaseUpdateProfile")}`;
                                ...
                                let response = `END ${getTranslation(lang, "farmingProfileDetails")}\n\n`;
                                // Handle primary crops safely
                                if (profile.FarmerPrimaryCrop &&
                                    profile.FarmerPrimaryCrop.length > 0) {
                                    const cropNames = profile.FarmerPrimaryCrop.filter((crop) => crop.product).map((crop) => crop.product.productName);
                                    response += `${getTranslation(lang, "primaryCrops")}: ${cropNames.join(", ") || getTranslation(lang, "none")}\n`;
                                }
                                else {
                                    response += `${getTranslation(lang, "primaryCrops")}: ${getTranslation(lang, "none")}\n`;
                                }
                                // Handle farm information safely
                                response += `${getTranslation(lang, "farmSize")}: ${profile.FarmerProfile.farmSize || 0} ${getTranslation(lang, "hectares")}\n`;
                                response += `${getTranslation(lang, "experience")}: ${profile.FarmerProfile.experienceYears || 0} ${getTranslation(lang, "years")}\n`;
                                if (profile.FarmerProfile.cooperativeName) {
                                    response += `${getTranslation(lang, "cooperativeName")}: ${profile.FarmerProfile.cooperativeName}\n`;
                                }
                                if (profile.FarmerProfile.cooperativeMember) {
                                    response += `${getTranslation(lang, "cooperativeMember")}: ${getTranslation(lang, "yes")}\n`;
                                }
                                else {
                                    response += `${getTranslation(lang, "cooperativeMember")}: ${getTranslation(lang, "no")}\n`;
                                }
                                return response;
                            }
                            catch (error) {
...
0. ${getTranslation(lang, "back")}`;
                            ...
                            if (parts.length === 4) {
                                ...
                                const pinResult = await verifyUserPin(phoneNumber, pinInput);
                                if (!pinResult.isValid) {
                                    return `END ${getTranslation(lang, pinResult.message || "incorrectPinMetrics")}`;
                                }
                                try {
                                    const metrics = await analyticsEarnings_service_1.AnalyticsEarningsService.getPerformanceMetrics(phoneNumber);
                                    if (!metrics) {
                                        return `END ${getTranslation(lang, "metricsDataNotAvailable")}`;
                                    }
                                    let response = `END ${getTranslation(lang, "performanceMetricsDetails")}\n\n`;
                                    response += `${getTranslation(lang, "acceptanceRate")}: ${metrics.acceptanceRate?.toFixed(2) || 0}%\n`;
                                    response += `${getTranslation(lang, "avgPricePerKg")}: ${metrics.avgPrice?.toFixed(2) || 0} RWF\n`;
                                    if (metrics.topProducts && metrics.topProducts.length > 0) {
                                        response += `${getTranslation(lang, "topProduct")}: ${metrics.topProducts[0].productName} (${metrics.topProducts[0]._sum.totalAmount || 0} RWF)\n`;
                                    }
                                    return response;
                                }
                                catch (error) {
...
0. ${getTranslation(lang, "back")}`;
                            ...
                            if (parts.length === 4) {
                                ...
                                const pinResult = await verifyUserPin(phoneNumber, pinInput);
                                if (!pinResult.isValid) {
                                    return `END ${getTranslation(lang, pinResult.message || "incorrectPinEarnings")}`;
                                }
                                try {
                                    const analytics = await analyticsEarnings_service_1.AnalyticsEarningsService.getComparisonAnalytics(phoneNumber);
                                    if (!analytics) {
                                        return `END ${getTranslation(lang, "noDataAvailable")}`;
                                    }
                                    let response = `END ${getTranslation(lang, "comparisonAnalytics")}\n\n`;
                                    response += `${getTranslation(lang, "regionalAverage")}: ${analytics.regionalAverage || 0} RWF\n`;
                                    response += `${getTranslation(lang, "yearlyGrowth")}: ${analytics.previousYear?.growthRate?.toFixed(2) || 0}%\n`;
                                    response += `${getTranslation(lang, "marketPosition")}: #${analytics.marketPosition || "N/A"}\n`;
                                    return response;
                                }
                                catch (error) {
...
0. ${getTranslation(lang, "back")}`;
                            ...
                            if (parts.length === 4) {
                                ...
                                try {
                                    const paymentHistory = await analyticsEarnings_service_1.AnalyticsEarningsService.getPaymentHistory(phoneNumber, 5);
                                    // Proper type checking for payment history
                                    if (!paymentHistory ||
                                        !Array.isArray(paymentHistory.recentPayments) ||
                                        paymentHistory.recentPayments.length === 0) {
                                        return `END ${getTranslation(lang, "noDataAvailable")}`;
                                    }
                                    // Type assertion for payment history
                                    const paymentData = paymentHistory;
                                    let response = `END ${getTranslation(lang, "paymentHistory")}\n\n`;
                                    paymentData.recentPayments.forEach((payment, index) => {
                                        response += `${index + 1}. ${payment.productName} - ${payment.totalAmount} RWF\n`;
                                        response += `   ${new Date(payment.paidAt).toLocaleDateString()}\n\n`;
                                    });
                                    response += `\n${getTranslation(lang, "pendingPayments")}: ${paymentData.pendingCount} (${paymentData.pendingAmount} RWF)`;
                                    return response;
                                }
                                catch (error) {
...
0. ${getTranslation(lang, "back")}`;
                                ...
                                try {
                                    await pinManagement_service_1.PinManagementService.changePIN(phoneNumber, (currentPin = ""), confirmPin);
                        ...
                        if (parts[2] === "2") {
                            try {
                                const activities = await activityMonitoring_service_1.ActivityMonitoringService.getRecentActivity(phoneNumber);
                                let response = `END ${getTranslation(lang, "recentActivity")}\n\n`;
                                activities.forEach((activity, index) => {
                                    const date = new Date(activity.timestamp).toLocaleDateString();
                                    response += `${index + 1}. ${getTranslation(lang, activity.action)} - ${date}\n`;
                                });
                                return response;
                            }
                            catch (error) {
...
0. ${getTranslation(lang, "back")}`;
                            ...
                            const pinResult = await verifyUserPin(phoneNumber, pinInput);
                            if (!pinResult.isValid) {
                                return `END ${getTranslation(lang, pinResult.message || "incorrectPasswordLanguage")}`;
                            }
                            return `CON ${getTranslation(lang, "selectLanguage")}
1. Kinyarwanda
2. English
